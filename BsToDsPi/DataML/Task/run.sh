...
PARTICLE="bs"           # Default value for PARTICLE
CHARMDAUGHTER="ds"
JOBS=1
DELETE=0
...
JSONCONFIG=dpl-config_mc.json
FILENAME="input_mc.txt"
if [ $MC -eq 0 ]; then
    JSONCONFIG=dpl-config.json
    FILENAME="./../../input_aod_reduced.txt"
fi
...
echo $PWD
echo ${INPUTNAMES[@]}
echo parallel -j "$JOBS" 'mkdir -p output_{#} && cd output_{#} && o2-analysis-hf-task-'"$PARTICLE"'-reduced '"$COMMONCONFIGS"' | o2-analysis-hf-candidate-selector-'"$PARTICLE"'-to-ds-pi-reduced '"$COMMONCONFIGS"' | o2-analysis-hf-candidate-creator-'"$PARTICLE"'-reduced '"$COMMONCONFIGS"' --aod-writer-json ../../../OutputDirector_task.json --resources-monitoring 2 --fairmq-ipc-prefix . --aod-file @../{1} > log_{#}.txt 2>&1 && cd ..' ::: ${INPUTNAMES[@]} 
