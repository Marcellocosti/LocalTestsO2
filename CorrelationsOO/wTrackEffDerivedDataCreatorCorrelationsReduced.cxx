...
struct HfDerivedDataCreatorCorrelationsReduced {
  ...
  using CollsWithCentMultMc = soa::Join<aod::Collisions, aod::EvSels, aod::FT0Mults, aod::FV0Mults, aod::CentFT0Ms, aod::CentFT0As, aod::CentFT0Cs, aod::CentFV0As, aod::McCollisionLabels>;
 
  using CandDsData = soa::Filtered<soa::Join<aod::HfCand3Prong, aod::HfSelDsToKKPi, aod::HfMlDsToKKPi>>;
  using CandDsMc = soa::Filtered<soa::Join<aod::HfCand3Prong, aod::HfSelDsToKKPi, aod::HfMlDsToKKPi, aod::HfCand3ProngMcRec>>;
 
  using CandDplusData = soa::Filtered<soa::Join<aod::HfCand3Prong, aod::HfSelDplusToPiKPi, aod::HfMlDplusToPiKPi>>;
  using CandDplusMc = soa::Filtered<soa::Join<aod::HfCand3Prong, aod::HfSelDplusToPiKPi, aod::HfMlDplusToPiKPi, aod::HfCand3ProngMcRec>>;
 
  using CandD0Data = soa::Filtered<soa::Join<aod::HfCand2Prong, aod::HfSelD0, aod::HfMlD0>>;
  using CandD0Mc = soa::Filtered<soa::Join<aod::HfCand2Prong, aod::HfSelD0, aod::HfMlD0, aod::HfCand2ProngMcRec>>;
 
  using TracksData = soa::Filtered<soa::Join<aod::TracksWDca, aod::TrackSelection, aod::TracksExtra>>;
  using TracksMcRec = soa::Filtered<soa::Join<aod::TracksWDca, aod::TrackSelection, aod::TracksExtra, o2::aod::McTrackLabels>>;
  using TracksMcGen = soa::Join<o2::aod::McParticles, aod::TracksExtra>;
  ...
  Preslice<TracksData> trackPerColl = aod::track::collisionId;
  Preslice<TracksMcRec> trackPerColl = aod::track::collisionId;
  Preslice<TracksMcGen> trackMcGenPerColl = o2::aod::mcparticle::mcCollisionId;
  ...
  void init(InitContext&)
  {
    ...
    if (doprocessTrackingEfficiency) {
      registry.add("hSparseGenTracks", "Tracking efficiency vs p "Tracking efficiency vs eta "Tracking efficiency phi ,T",::  {HistType, {axisPtAssoc, axisEta, axisPhi, axisCent}});
    } else {
  ...
  void fillTrigCharmInfo(TTrigCands const& trigCands)
  {
    for (const auto& trigCand : trigCands) {
  ...
  void fillAssocTrkInfo(TAssocTrks const& assTrks,
                         const float collCentrality)
  {
    bool first = true;
    for (const auto& assTrk : assTrks) {
                              ...
                              TracksData const& tracks)
  {
    ...
    rowCollisions(mult, coll.numContrib(), cent, coll.posZ());
    fillTrigCharmInfo<CandType::DplusToPiKPi>(candsDplus);
    fillAssocTrkInfo(tracks, cent);
  }
                           ...
                           CandDsData const&)
  {
    ...
    rowCollisions(mult, coll.numContrib(), cent, coll.posZ());
    fillTrigCharmInfo<CandType::DsToPiKK>(candsDsToPiKK);
    fillTrigCharmInfo<CandType::DsToKKPi>(candsDsToKKPi);
    fillAssocTrkInfo(tracks, cent);
  }
                           ...
                           CandD0Data const&)
  {
    ...
    rowCollisions(mult, coll.numContrib(), cent, coll.posZ());
    fillTrigCharmInfo<CandType::D0ToPiK>(candsD0ToPiK);
    fillTrigCharmInfo<CandType::D0ToKPi>(candsD0ToKPi);
    fillAssocTrkInfo(tracks, cent);
  }
  ...
  void processTrackingEfficiency(CollsWithCentMultMc::iterator const& recColl,
                                 TracksMcRec const& recTracks,
                                 TracksMcGen const& genTracks)
  {
    float cent{-1.}, mult{-1.};
    if (!checkCollision(recColl, cent, mult)) {
      return;
    }

    auto genColl = recColl.template mcCollision_as<aod::McCollisions>();
    const auto recCollsThisGenColl = recColls.sliceBy(collPerCollMc, genColl.globalIndex());
    if (recCollsThisGenColl.size() > 1) {
      LOG(warn) << "More than one reconstructed collision for the same MC collision, skipping the efficiency calculation for this collision";
      return;
    }
    rowCollisions(mult, recColl.numContrib(), cent, recColl.posZ());

    const auto recTracksThisColl = recTracks.sliceBy(collPerCollMc, recColl.globalIndex());
    fillAssocTrkInfo(recTracksThisColl, cent);

    const auto genTracksThisColl = genTracks.sliceBy(perCollisionMc, genColl.globalIndex());
    for (const auto& genTrk : genTracksThisColl) {
      if (nabs(genTrk.eta()) > etaTrkMax || genTrk.pt() < ptTrkMin || genTrk.pt() > ptTrkMax) {
        continue;
      }
      registry.fill(HIST("hSparseGenTracks"), genTrk.pt(), genTrk.eta(), genTrk.phi(), centrality);
    }
  }
  PROCESS_SWITCH(HfDerivedDataCreatorCorrelationsReduced, processTrackingEfficiency, "Process tracking efficiency", false);
