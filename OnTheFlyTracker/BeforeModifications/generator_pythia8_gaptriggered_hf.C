...
protected:
  ...
  {
    std::cout << "***********************************************" << std::endl;
    std::cout << "Generating event number " << mGeneratedEventsTotal << ", accepted: " << mGeneratedEvents << std::endl;
    // Simple straightforward check to alternate generators
    if (mGeneratedEvents % mInverseTriggerRatio == 0)
    {
      // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Generating event of interest (event number " << mGeneratedEvents << ")" << std::endl;
      int nInjectedEvents = mGeneratedEvents / mInverseTriggerRatio;
      // Alternate quarks i\f enabled (with the same ratio)
      if (mQuarkPdgList.size() >= 1)
      {
        int iQuark = nInjectedEvents % mQuarkPdgList.size();
        mQuarkPdg = mQuarkPdgList[iQuark];
        // LOG(info)<<"[mGenTot = " << mGeneratedEventsTotal << "] SELECTED quark: "<<mQuarkPdgList[iQuark];
      }
      ...
      if (mHadronPdgList.size() >= 1)
      {
        ...
        // LOG(info)<<"[mGenTot = " << mGeneratedEventsTotal << "] SELECTED hadron: "<<mHadronPdgList[iHadron];
      }
    ...
    {
      // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Generating minimum-bias event (event number " << mGeneratedEvents << ")" << std::endl;
  ...
  bool selectEvent()
  {
    ...
    // std::cout << "isGoodAtPartonLevel: " << isGoodAtPartonLevel << ", isGoodAtHadronLevel: " << isGoodAtHadronLevel << ", anyPartToReplace: " << anyPartToReplace << std::endl;
    ...
    for (auto iPart{0}; iPart < mPythia.event.size(); ++iPart)
    {
      ...
      if (!isGoodAtPartonLevel)
      {
        ...
        for (auto iDau : daughterList)
        {
          if (mPythia.event[iDau].id() == mQuarkPdg)
          {
            ...
            {
              // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Found quark " << mQuarkPdg << " at rapidity " << mPythia.event[iDau].y() << std::endl;
              atSelectedY = true;
            }
          }
          if (mPythia.event[iDau].id() == -mQuarkPdg)
          {
            ...
            if ((mPythia.event[iDau].y() > mQuarkRapidityMin) && (mPythia.event[iDau].y() < mQuarkRapidityMax))
            {
              // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Found antiquark " << -mQuarkPdg << " at rapidity " << mPythia.event[iDau].y() << std::endl;
              atSelectedY = true;
            }
          }
        }
        // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] About to check isGoodAtPartonLevel ... " << std::endl;
        if (hasQ && hasQbar && atSelectedY)
        {
          // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Found good particle at parton level!" << std::endl;
          // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Found quark pair " << mQuarkPdg << " at rapidity " << mPythia.event[iPart].y() << std::endl;
          isGoodAtPartonLevel = true;
        }
        // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Checked isGoodAtPartonLevel = " << isGoodAtPartonLevel << std::endl;
      }
      // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] [isGoodAtHadronLevel = " << isGoodAtHadronLevel << "] Checking particle " << mPythia.event[iPart].id() << " at rapidity " << mPythia.event[iPart].y() << std::endl;
      ...
      if (!isGoodAtHadronLevel)
      {
        ...
        // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Checking particle " << id << " vs " << mHadronPdg << ", rap = " << rap << " vs " << mHadRapidityMin << " and " << mHadRapidityMax << std::endl;
        if (id == mHadronPdg && rap > mHadRapidityMin && rap < mHadRapidityMax)
        {
          // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Found good particle at hadron level, pdg: " << id << ", rapidity: " << rap << std::endl;
          // std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Found hadron " << id << " at rapidity " << rap << std::endl;
      ...
      for (auto iPartToReplace{0u}; iPartToReplace<mPartPdgToReplaceList.size(); ++iPartToReplace) {
        if (std::abs(mPythia.event[iPart].id()) == mPartPdgToReplaceList[iPartToReplace][0] && pseudoRndm < mFreqReplaceList[iPartToReplace]) {
          LOGP(info, "REPLACING PARTICLE {} WITH {}, BEING RNDM {}", mPartPdgToReplaceList[iPartToReplace][0], mPartPdgToReplaceList[iPartToReplace][1], pseudoRndm);
          replaceParticle(iPart, mPartPdgToReplaceList[iPartToReplace][1]);
          break;
        }
      }

      // if (std::abs(mPythia.event[iPart].id()) == 4122) {
      //   std::cout << "[mGenTot = " << mGeneratedEventsTotal << "] Checked particle " << mPythia.event[iPart].id() << " at rapidity " << mPythia.event[iPart].y() << ": isGoodAtPartonLevel = " << isGoodAtPartonLevel << ", isGoodAtHadronLevel = " << isGoodAtHadronLevel << ", anyPartToReplace = " << anyPartToReplace << std::endl;
      // }
      ...
      {
        std::cout<<"[mGenTot = " << mGeneratedEventsTotal << "] EVENT SELECTED: Found particle "<<mPythia.event[iPart].id()<<" at rapidity "<<mPythia.event[iPart].y()<<" for quark "<<mQuarkPdg<<" and hadron "<<mHadronPdg<<std::endl;
        return true;
      }
    }
    ...
    if (isGoodAtPartonLevel && isGoodAtHadronLevel) {
      // std::cout << "EVENT SELECTED: No particle to replace, but found quark and hadron at selected rapidity\n";
      return true;
    }

    // std::cout << "EVENT REJECTED: isGoodAtPartonLevel = " << isGoodAtPartonLevel << ", isGoodAtHadronLevel = " << isGoodAtHadronLevel << "\n";
    return false;
  };
  ...
  unsigned long long mGeneratedEventsTotal;
